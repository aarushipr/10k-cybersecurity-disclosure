...
# table: descriptive statistics: overall + by year
rows = []
for label, subset in [("All years", df)] + [
...
# Fig: Boilerplate RATIO over time (primary finding: it's tiny)
...
for year in sorted(df["year"].unique()):
    ...
    ax1.scatter(
        np.full(len(vals), year) + jitter, vals, color=BLUE, alpha=0.20, s=22, zorder=2
    )
...
# Fig: Count AND ratio: with vs without Item 1C (side-by-side panels)
#          Shows count rises but ratio stays flat
ic = df.groupby("has_1c")[["boilerplate_count", "boilerplate_ratio"]].agg(
    ["mean", "std"]
)
labels = ["Without\nItem 1C", "With\nItem 1C"]
counts = [ic["boilerplate_count"]["mean"][False], ic["boilerplate_count"]["mean"][True]]
count_sds = [
    ic["boilerplate_count"]["std"][False],
    ic["boilerplate_count"]["std"][True],
]
ratios = [ic["boilerplate_ratio"]["mean"][False], ic["boilerplate_ratio"]["mean"][True]]
ratio_sds = [
    ic["boilerplate_ratio"]["std"][False],
    ic["boilerplate_ratio"]["std"][True],
]
...
bars = ax_c.bar(
    ...
    width=0.4,
    capsize=6,
    error_kw=dict(ecolor=GRAY, lw=1.3),
)
...
fig.suptitle(
    "Boilerplate Count vs Ratio: Filings With vs Without Item 1C",
    fontweight="bold",
    fontsize=11,
)
plt.tight_layout()
...
# Fig: Boilerplate count by sector
sec = (
    df.groupby("sector")["boilerplate_count"].agg(["mean", "std"]).reindex(SECTOR_ORDER)
)
...
# Fig: Boilerplate count AND ratio by firm size (side-by-side panels)
