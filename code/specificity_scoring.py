...
content = pd.read_excel(
    os.path.join(SCRIPT_DIR, "..", "results", "content_scores.xlsx")
)
boiler = pd.read_csv(
    os.path.join(SCRIPT_DIR, "..", "results", "boilerplate_results.csv")
)
...
df["category"] = df["S"].apply(
    lambda x: "High Specificity" if x >= 60 else "Low Specificity"
)
...
# descriptive stats table + category split by year
rows = []
...
# Fig:  Mean S over time (left) + stacked category share (right)
...
for year in years:
    ...
    ax1.scatter(
        np.full(len(vals), year) + jitter, vals, color=BLUE, alpha=0.22, s=22, zorder=2
    )
...
# Fig: – S distribution: with vs without Item 1C (KDE + histogram)
...
# Fig: S score by firm size (boxplot)
