...
def run_pipeline(
    ...
    if "year" in df.columns:
        df["_firm_year"] = (
            df[firm_col].astype(str) + " (" + df["year"].astype(str) + ")"
        )
        id_col = "_firm_year"
    else:
        id_col = firm_col
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        stat_cols = [f"w_{f}" for f in FUNCTIONS] + [
            "balance_score",
            "total_keyword_hits",
        ]
        results[stat_cols].describe().round(4).to_excel(
    ...
    for _, r in results.iterrows():
        ...
        print(
            f"  {r['firm_year']:<38} {r['balance_score']:>7.4f}   {ok}   {r['primary_function']:>7}"
        )
