...
def _save(existing, new_results, output_path):
    ...
    combined["specificity_score"] = (
        combined[SPECIFICITY_CATEGORIES].sum(axis=1) / TOTAL_MARKERS
    )
...
def run_content_scoring(data_dir=DATA_DIR, output_path=OUTPUT_PATH):
    ...
    if os.path.exists(f"{output_path}.parquet"):
        ...
        print(
            f"[INFO] Resuming — {len(done)} already scored, {len(filings) - len(done)} remaining"
        )
    else:
