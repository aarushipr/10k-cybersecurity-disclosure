...
# Fig: Average Function Weights (bar chart)
...
# Fig: Function Weights Over Time (stacked bar)
if "year" in df.columns and df["year"].nunique() > 1:
    ...
    ax.legend(
        loc="upper left", frameon=False, fontsize=9, ncol=2, bbox_to_anchor=(1.01, 1)
    )
    plt.tight_layout()
    save("fw_over_time")

# Fig: Function Weights by Sector (heatmap)
sectors = sorted(df_valid["sector"].dropna().unique())
sector_weights = (
    df_valid.groupby("sector")[w_cols]
    .mean()
...
sns.heatmap(
    ...
    vmin=0,
    vmax=sector_weights.values.max(),
)
ax.set_xlabel("")
ax.set_ylabel("")
...
# Fig: Function Weights by Firm Size (grouped bar)
