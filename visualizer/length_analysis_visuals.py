...
# Table: descriptive statistics for Item 1A and Item 1C by year
rows = []
...
# Fig: Item 1C adoption rate by year
...
# Fig: Item 1C mean length over time with ±1 SD bands (2023–2025)
temporal = df_1c.groupby("year")["len_1c"].agg(["mean", "std", "count"]).reset_index()
...
# Fig: Dual distribution: Item 1A (all years) and Item 1C (2024–2025)
...
# Fig: Dual boxplot: 1A and 1C by firm size
...
# Fig: Mean section length over time: Item 1A (left) and 1C (right axis)
...
ax2.plot(
    ...
    lw=2.4,
    ls="--",
    label="Item 1C (filings with 1C only)",
)
...
ax1.text(
    2023.3,
    ax1.get_ylim()[0] if ax1.get_ylim()[0] > 0 else mean_1a.min() * 0.9995,
    "SEC Rule\neffective",
    color=GRAY,
    fontsize=9,
    va="bottom",
...
# Fig: Mean section length by sector: Item 1A and 1C (grouped horizontal bars)
SECTOR_ORDER_LEN = [
    "Healthcare",
    "Consumer Goods",
    "Semiconductors",
...
# Fig: Scatter: Item 1A vs Item 1C word count, coloured by sector
