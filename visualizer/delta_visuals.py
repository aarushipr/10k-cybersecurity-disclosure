...
for delta_val in [0.0, 0.5, 1.0]:
    ...
    bars = ax.bar(
        ...
        width,
        bottom=bottom,
        color=DELTA_COLORS[delta_val],
        alpha=0.88,
        edgecolor="white",
        label=DELTA_LABELS[delta_val],
    )
...
# Fig: delta distribution by Sector (Stacked % Bar)
sec_delta = (
    df_delta.groupby(["sector", "delta"])
    .size()
