...
MANUAL_OVERRIDES = {
    ("ELMD", 2022): (
        "https://www.sec.gov/ix?doc=/Archives/edgar/data/0001488917/000089710122000805/elmd221010_10k.htm",
        None,
    ),
    ("ELMD", 2023): (
        "https://www.sec.gov/ix?doc=/Archives/edgar/data/0001488917/000089710123000380/elmd230887_10k.htm",
        None,
    ),
    ("ELMD", 2024): (
        "https://www.sec.gov/ix?doc=/Archives/edgar/data/0001488917/000089710124000422/elmd240883_10k.htm",
        None,
    ),
    ("ELMD", 2025): (
        "https://www.sec.gov/ix?doc=/Archives/edgar/data/0001488917/000143774925027761/elmd20250630_10k.htm",
        None,
    ),
    ("NVEC", 2022): (
        "https://www.sec.gov/ix?doc=/Archives/edgar/data/0000724910/000143774922010855/nvec20220331_10k.htm",
        None,
    ),
    ("NVEC", 2024): (
        "https://www.sec.gov/ix?doc=/Archives/edgar/data/0000724910/000137647424000214/nvec-20240331.htm",
        None,
    ),
    ("NVEC", 2025): (
        "https://www.sec.gov/ix?doc=/Archives/edgar/data/0000724910/000137647425000437/nvec-20250331.htm",
        None,
    ),
}
            ...
            try:
                ...
                if not is_valid_text(item_1a_text):
                    ...
                    status = (
                        "raw fallback OK"
                        if item_1a_text
                        else "FAILED — add to MANUAL_OVERRIDES"
                    )
                ...
                if year >= 2023:
                    ...
                    if override and override[1]:
                        ...
                        if item_1c_text:
                            print(
                                f"        {ticker} {year} Item 1C: manual override OK"
                            )
                    ...
                    if not is_valid_text(item_1c_text):
                        try:
                            item_1c = doc["Item 1C"]
                            item_1c_text = (
                                str(item_1c) if is_valid_text(item_1c) else None
                            )
                        except Exception:
                            pass
                ...
                if len(parts) == 2:
                    ...
                    combined = None
                    print(
                        f"[WARN]  {ticker} {year}: no text extracted — add URL to MANUAL_OVERRIDES"
                    )
...
def save_outputs(records):
    ...
    if not empty.empty:
        ...
        for _, row in empty.iterrows():
            print(
                f"  ({row['ticker']!r}, {row['year']}): search https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&type=10-K&dateb=&owner=include&count=40&search_text=&company={row['ticker']}"
            )
